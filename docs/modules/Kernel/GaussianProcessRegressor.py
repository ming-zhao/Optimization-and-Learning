...
class GaussianProcessRegressor(object):
    ...
    def fit(self, X, t, iter_max=0, learning_rate=0.1):
        ...
        self.covariance = Gram + I / self.beta # (6.62) 
        self.precision = np.linalg.inv(self.covariance)
        
        # maximize likelihood to update the kernel's parameter
    ...
    def predict(self, X, with_error=False):
        ...
        if X.ndim == 1:
            X = X[:, None]
        K = self.kernel(X, self.X) # self.X is trainning data, X is new input
        ...
        if with_error:
            ...
            return mean.ravel(), np.sqrt(var.ravel()) # (6.67)
        return mean
