...
class PCA(object):
    ...
    def eigen(self, X, *arg):
        ...
        if sample_size >= n_features:
            ...
            cov = np.cov(X)
            values, vectors = np.linalg.eigh(cov) # return eigenvalue from smallest to largest
            vectors = (X.T @ vectors) / np.sqrt(sample_size * values.clip(min=1e-10))  #(12.30)
            index = sample_size - self.n_components
        self.I = np.eye(self.n_components)
        if index == 0:
            self.var = 0
        else:
            self.var = np.mean(values[:index])                     #(12.46)

        self.W = vectors[:, index:].dot(np.sqrt(np.diag(values[index:]) - self.var * self.I)) # (12.45) with R = Identity Matrix
        self.__M = self.W.T @ self.W + self.var * self.I           #(12.41)
        self.C = self.W @ self.W.T + self.var * np.eye(n_features) #(12.36)
        if index == 0:
            self.Cinv = np.linalg.inv(self.C)
        else:
            self.Cinv = np.eye(n_features) / np.sqrt(self.var) - self.W @ np.linalg.inv(self.__M) @ self.W.T / self.var #(12.40)
    ...
    def _expectation(self, X):
        ...
        Ez = X @ self.W @ Minv #(12.54)
        Ezz = self.var * Minv + Ez[:, :, None] * Ez[:, None, :] #(12.55)
        return Ez, Ezz

    def _maximization(self, X, Ez, Ezz):
        self.W = X.T @ Ez @ np.linalg.inv(np.sum(Ezz, axis=0)) #(12.56)
        self.var = np.mean(
            np.mean(X ** 2, axis=1) - 2 * np.mean(Ez @ self.W.T * X, axis=1)
            + np.trace((Ezz @ self.W.T @ self.W).T) / np.size(X, 1)) #(12.57)
    ...
    def transform(self, X):
        ...
        """
        return np.linalg.solve(self.__M, ((X - self.mean) @ self.W).T).T  #(12.48)
    ...
    def prob(self, X):
        ...
        return (
            ...
            / np.power(2 * np.pi, 0.5 * np.size(X, 1))) #(12.35)
