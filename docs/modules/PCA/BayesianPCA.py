...
class BayesianPCA(PCA):
    ...
    def fit(self, X, iter_max=100, initial="random"):
        ...
        Parameters
        ----------
        X : (sample_size = N, n_features = D) ndarray
            input data
        iter_max : int
        ...
        for i in range(iter_max):
            W = np.copy(self.W)
            stats = self._expectation(X - self.mean)   # obtain Ez = E[z], Ezz=E[zz]
            self._maximization(X - self.mean, *stats)
            self.alpha = len(self.mean) / np.sum(self.W ** 2, axis=0).clip(min=1e-10) #(12.62)
            if np.allclose(W, self.W):
                break
        self.n_iter = i + 1

    def _maximization(self, X, Ez, Ezz):
        self.W = X.T @ Ez @ np.linalg.inv(np.sum(Ezz, axis=0) + self.var * np.diag(self.alpha)) #(12.63)
        self.var = np.mean(
            np.mean(X ** 2, axis=-1)
            - 2 * np.mean(Ez @ self.W.T * X, axis=-1)
            + np.trace((Ezz @ self.W.T @ self.W).T) / len(self.mean))  # (12.57) len(self.mean) = D
